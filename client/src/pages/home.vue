  ...
  methods: {
    ...
    upLoadFun(item) {
      ...
      fr.onload = (data) => {
        ...
        //检查文件
        this.$http.post(`${this.$store.state.serve.serveUrl}upload/examineFile`, {
                ...
                if (!examineres.data.data.fileExist) {
                  ...
                  for (let i = 0, len = chunks; i < len; i++) {
                    ...
                    //片段流上传
                      ...
                      this.$http.put(`${this.$store.state.serve.serveUrl}upload/uploadStreamFile?userid=${encodeURI(this.$store.state.id).replace(/\+/g,'%2B')}&folderid=${encodeURI(item.folderId).replace(/\+/g,'%2B')}&fileName=${item.fname}&filePath=${item.filePath}&fileExt=${item.fext}&fileSha256=${item.fileSha256}&currentChunkMax=${chunks}&currentChunkIndex=${i}`,
                        fileslice,
                      {
                        headers: {
                        ...
                        if (item.uploadCurrentChunkNum >= item.currentChunkMax) {
                          ...
                          this.$refs.childRouter.getUserFileAndFolder(this.$refs.childRouter.getFolderId);
                          //设置上传完成
